//! Conversion between framework types and Foundry Local's OpenAI-compatible
//! `/v1/chat/completions` wire format.
//!
//! Foundry Local's OpenAI-compatibility layer
//! (<https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-local/reference/reference-catalog-api#openai-compatibility>)
//! speaks the *exact* same JSON shapes as OpenAI's Chat Completions API for
//! everything this crate needs: `messages`, `tools`/`tool_choice`, the scalar
//! sampling options, the non-streaming response envelope, `usage`, and the
//! `chat.completion.chunk` SSE delta shape. So request-body assembly and
//! response/usage parsing are reused verbatim from
//! [`agent_framework_openai::convert`] rather than duplicated (mirrors how
//! `agent-framework-ollama` reuses the same module, and for the same reason).
//! Only the streaming-delta parser is reimplemented locally, kept small on
//! purpose, so it can be unit-tested here over fixture JSON without going
//! through a live `reqwest::Response`.
//!
//! Known gaps versus upstream OpenAI, reflecting real Foundry Local server
//! behavior: no `refusal` field, no `completion_tokens_details` /
//! `prompt_tokens_details` breakdown (harmless — `parse_usage` simply finds
//! nothing there), and no `system_fingerprint`. None of these affect the
...
/// delegated to [`agent_framework_openai::convert`] since Foundry Local's
/// OpenAI compatibility layer accepts the identical shapes.
pub fn build_request(
    ...
    if stream {
        ...
        // Foundry Local honors `stream_options.include_usage` the same way
        // OpenAI does: the final chunk carries a top-level `usage` object.
...
mod tests {
    ...
    fn build_request_includes_model_and_messages() {
        ...
        assert_eq!(body["model"], json!("phi-3.5-mini"));
        assert_eq!(
            body["messages"],
    ...
    fn build_request_applies_temperature_and_max_tokens() {
        ...
        let body = build_request(&[Message::user("hi")], &options, "phi-3.5-mini", false);
    ...
    // exercised here against Foundry Local-shaped fixtures)

    #[test]
    fn parses_foundry_local_chat_completion_response() {
        let value = json!({
            "id": "chatcmpl-123",
            "model": "phi-3.5-mini",
            "choices": [{
    ...
    #[test]
    fn parses_foundry_local_tool_call_response() {
        let value = json!({
            "id": "chatcmpl-456",
            "model": "phi-3.5-mini",
            "choices": [{
