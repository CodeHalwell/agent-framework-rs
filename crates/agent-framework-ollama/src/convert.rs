//! Conversion between framework types and Ollama's OpenAI-compatible
//! `/v1/chat/completions` wire format.
//!
//! Ollama's OpenAI-compatibility layer (<https://github.com/ollama/ollama/blob/main/docs/openai.md>)
...
//! `agent-framework-azure` reuses the same module for Azure OpenAI). Only the
//! streaming-delta parser is reimplemented locally, kept small on purpose, so
//! it can be unit-tested here over fixture JSON without going through a live
//! `reqwest::Response`.
//!
//! Known gaps versus upstream OpenAI, reflecting real Ollama server
...
/// delegated to [`agent_framework_openai::convert`] since Ollama's OpenAI
/// compatibility layer accepts the identical shapes.
pub fn build_request(
    ...
    if stream {
        ...
        // Ollama honors `stream_options.include_usage` the same way OpenAI
        // does: the final chunk carries a top-level `usage` object.
...
/// (Ollama's SSE chunks are byte-for-byte the same shape) — reimplemented
...
mod tests {
    ...
    fn build_request_applies_temperature_and_max_tokens() {
        ...
        let body = build_request(&[Message::user("hi")], &options, "llama3.1", false);
    ...
    fn build_request_includes_tools_and_tool_choice() {
        ...
        let body = build_request(&[Message::user("weather?")], &options, "llama3.1", false);
    ...
    // exercised here against Ollama-shaped fixtures)

    #[test]
    fn parses_ollama_chat_completion_response() {
        let value = json!({
            "id": "chatcmpl-123",
            "model": "llama3.1",
            "choices": [{
