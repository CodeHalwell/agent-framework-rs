...
| Azure AI Search / ChatKit / `lab` extras | ✅ (various packages) | ✅ (various packages) | ❌ not yet | not individually tracked in this matrix; CosmosDB has moved to its own row under Middleware & memory (🚧 partial, master-key auth only) |
...
- **Ecosystem**: ChatKit, Azure AI Search, DurableTask/Azure Functions hosting, OTel SDK exporter wiring (AG-UI, Cosmos DB, CopilotStudio, and Purview now ship — see Serving and Middleware & memory).
