...
| Purview / ChatKit / CosmosDB / `lab` extras | ✅ (various packages) | ✅ (various packages) | ❌ not yet | not individually tracked in this matrix |
...
- **Azure**: the Azure AI Foundry (persistent agents) client, the Entra ID credential chain, and the Azure AI Search context provider now ship; route/`api-version` fidelity for the Foundry data plane follows the documented Assistants convention (not locally verifiable against a vendored SDK).
- **Ecosystem**: CopilotStudio, Purview, ChatKit, CosmosDB, DurableTask/Azure Functions hosting, OTel SDK exporter wiring (AG-UI now ships — see Serving; Azure AI Search now ships — see Chat clients & providers).
