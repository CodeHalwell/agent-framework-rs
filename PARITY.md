...
| Tool-loop bounds (`max_iterations` / `max_function_calls` / `max_duration_seconds`) | ✅ (#7772) | 🚧 | ✅ done | all three on `FunctionInvocationConfig`, reachable from an agent via `AgentBuilder::function_invocation_config`. The call and wall-clock budgets are **cumulative across approval round trips** (parked in `AgentSession::state`, so a run that pauses for a human cannot reset its own budget), graceful (tools disabled, the model answers with what it has), and a *ceiling*: once spent the loop stops executing rather than only setting `tool_choice`, which a provider is free to ignore |
...
| Azure AI Foundry service client | ✅ (`foundry`) | ✅ (`foundry` package: `FoundryChatClient`/`FoundryAgent`) | ✅ done | `agent-framework-foundry::FoundryChatClient` — Foundry project **Responses API** (`POST {endpoint}/openai/v1/responses`, path-versioned, no `?api-version=` query), delegating request/response conversion, SSE streaming, and error classification to `agent_framework_azure::responses::AzureOpenAIResponsesClient` rather than re-implementing them. `FoundryAgent` realizes a Prompt Agent client-side (`to_prompt_agent()` ↔ `PromptAgentDefinition`); it does not (yet) bind to a server-hosted agent by id on the Foundry Agents control plane. Entra ID auth via `TokenCredential` (scope `https://ai.azure.com/.default`, `FOUNDRY_SCOPE`). Supersedes the removed Azure AI Agents threads/runs data plane this crate used to speak directly (`agent-framework-azure-ai::AzureAIAgentClient`) |
| Anthropic Messages API | ✅ | ✅ | ✅ done | `agent-framework-anthropic::AnthropicClient`, hand-rolled (no Anthropic SDK dependency) |
| Anthropic extended-thinking signatures | ✅ (#8171) | not verified | ✅ done | a thinking block's `signature` rides in `TextReasoningContent::protected_data` on both the buffered and streaming paths (the streamed `signature_delta` is carried as an empty-text fragment so the existing `coalesce_text` merge lands it on the block it signs), and `redacted_thinking` is preserved rather than dropped. Outbound, a decoded block is replayed **verbatim** from its raw form — the signature covers the block's exact text — and an *unsigned* reasoning content is dropped rather than sent, because the API rejects one when extended thinking is on |
...
| MCP header scoping (credentials across redirects) | ✅ (#8285) | not verified | ✅ done | the transport follows redirects itself (`Policy::none()`) and re-attaches the configured headers and the `Mcp-Session-Id` only while the target is same-origin by scheme/host/port; `reqwest`'s own handling strips three well-known header names on a cross-*host* hop and passes everything else, which leaks an `X-Api-Key` and keeps `Authorization` across a port or scheme change. Method and body are preserved on every hop, where `reqwest` would degrade a 301/302/303 POST to a GET. A session id is only adopted from a same-origin response |
...
| Redis chat-message store | ✅ (`redis` package) | not found | ✅ done | `agent-framework-redis::RedisChatMessageStore` — one LIST per thread, JSON messages, optional trimming; close mirror of Python. Keys are derived through `core::storage_keys::storage_key_segment` (#8236), so a session id containing the separator cannot address another store's list; a literal-safe prefix and id — the default prefix and any UUID — are unchanged, so existing data is unaffected. Upstream's separate tenant/application/agent key segments are not ported: with an injective encoding a caller composes those into `key_prefix` and gets the same isolation |
...
| Cosmos DB chat-message store | ✅ (`azure-cosmos`) | ✅ (`Microsoft.Agents.AI.CosmosNoSql`) | 🚧 partial | `agent-framework-cosmos::CosmosChatMessageStore` (+ `CosmosCheckpointStorage`) — hand-rolled Cosmos DB NoSQL REST client (no `azure_data_cosmos`/`Microsoft.Azure.Cosmos` SDK dependency); one container, messages partitioned by `threadId`; `ensure_created()`, add/list/clear. **Both auth modes ship**: master key (HMAC-SHA256) and Entra ID via `with_token_credential(..)`, taking any `agent_framework_azure::TokenCredential` (so `DefaultAzureCredential`/managed identity work, and an account with `disableLocalAuth` is usable at all). Cosmos's Entra RBAC is data-plane-only, so `ensure_created` cannot succeed with a token whatever the role assignment — the call reports that instead of a bare `403`, and the container is provisioned out of band. Still open: no `TransactionalBatch` (one `POST` per message on multi-message adds); no hierarchical partition keys; no TTL |
...
| Azure AI Search vector store | ✅ (`AzureAISearchCollection`/`AzureAISearchStore`, #8153) | ✅ | ✅ done | `agent-framework-azure-ai-search::{AzureAISearchStore, AzureAISearchCollection}` — index create/exists/delete built from a `VectorStoreCollectionDefinition` (EDM types, vector profiles, hnsw vs exhaustiveKnn, metric mapping), document upsert/get/delete over `docs/index` (batched, and a **207** partial success is raised rather than read as success), vector search with the portable filter translated to OData, keyword-hybrid search (`search_hybrid`, reached through the concrete type since `VectorCollection` takes only a vector), and index aliases. Filtering is `preFilter` and `k` covers `skip + top`, so a selective filter does not return fewer than `top`. Operators Azure cannot express (`ne`, `exists`, `is_null`, `is_not_null`, literal `starts_with`/`ends_with`/`contains_text`) are refused with the alternative named, never dropped. REST, like the context provider beside it — no `azure-search-documents` SDK |
...
| Vector stores (field/collection definitions, collection + store traits) | ✅ (`_vectors`) | ✅ (CommunityToolkit.VectorData) | ✅ done | `agent-framework-core::vectors` — `VectorStoreField`/`VectorStoreCollectionDefinition` (validated at construction), `IndexKind`/`DistanceFunction` (+ `higher_is_closer`), `VectorSearchOptions`/`VectorSearchResult`, object-safe `VectorCollection`/`VectorStore` traits, and an `InMemoryVectorStore`. Records are `serde_json::Value` keyed by field name rather than a registered model type — `serde` makes upstream's decorator/annotation model layer redundant |
| Portable vector filters | ✅ (`_vector_filters`, #8115) | 🚧 (`VectorData` filter expressions) | ✅ done | `vectors::filters` — `Filter` leaves + `FilterGroup` nodes over 18 operators, validated at construction (operand shape per operator, depth ≤ 8, ≤ 64 nodes), namespaced provider operators (`azure_ai_search.match`), and an evaluator `InMemoryVectorStore` uses. Upstream's `Param` late-binding (a placeholder filled from an agent tool call, with a generated argument schema) is **not** ported: it exists because Python cannot type the call, which a Rust caller does with the arguments it already holds |
| Vector filter semantics (missing field, bool≠number, numeric equality) | ✅ | not verified | ✅ done | a missing field is a non-match for every operator except `exists` — so `ne` and `not(eq)` differ — a boolean never equals a number, and two numbers compare **by value**, so a record that round-tripped as `1` matches `eq: 1.0` (`serde_json`'s own `==` says it does not). Integers compare as integers: routing them through `f64` to get that cross-type rule would round past 2^53, where two adjacent 64-bit ids compare equal |
| Vector search score direction | ✅ (`score_kind` result metadata) | not verified | ✅ done | `VectorSearchResult::score_kind` + `higher_is_closer(..)`. A collection's distance function is a request, and on several services it does not describe what comes back — Azure AI Search returns a higher-is-better relevance score whatever metric the profile declares, and an RRF score for a hybrid query, so reading direction from the definition inverts the ranking |
...
- **Cosmos DB store**: one `POST` per message instead of `TransactionalBatch`; no hierarchical partition keys or TTL. (Entra ID auth now ships alongside master-key auth.)
- **Azure ecosystem, unported packages**: `azure-cosmos-memory` (Cosmos-backed memory context provider), `azure-contentunderstanding` (document ingestion + file search), `foundry_hosting` / .NET `Foundry.Hosting` (hosting *server-side* Foundry agents; the Rust Foundry crate is a client only), .NET `Hosting.AzureStorage` (Blob-backed session persistence), and .NET `Workflows.Declarative.Foundry` (rides on the declarative-workflow DSL divergence).
...
- **Purview**: covers only `processContent` — no protection-scopes precheck/caching, background content-activity logging, or JWT-derived identity fallback. (Every request now sends `Prefer: evaluateInline`, which is what the absent precheck would otherwise have selected.)
- **Vector stores**: Azure AI Search and the in-memory store implement the traits; upstream's PostgreSQL/pgvector, Qdrant and Redis HASH/JSON connectors (all landed 2026-09-09) do not have Rust counterparts yet. Each is now a straightforward translation job rather than a design one, since the portable filter they all consume ships.
